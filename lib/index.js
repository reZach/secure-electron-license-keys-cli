...
export function cli(args) {
  const options = parseArgumentsIntoOptions(args); // Define user license options/values
  ...
  const {
    publicKey,
    privateKey
  } = crypto.generateKeyPairSync("rsa", cryptoKeyPairOptions); // Sign user data with the private key

  const encrypted = crypto.privateEncrypt(privateKey, Buffer.from(JSON.stringify(userData))); // Save license data, along with public/private keys
